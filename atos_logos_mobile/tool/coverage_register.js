// Prints a per-file coverage table for the mobile register flow.
// Run from atos_logos_mobile/ root after `flutter test --coverage`.
//
// Mirrors the structure of `coverage_login.js`. The register flow shares
// most of its plumbing with login (auth_repository, auth_cubit, interceptor,
// router, models), so this report focuses on the register-only surface
// — the page itself, the validators helper, the SignupRequest model, and
// the auth_cubit/auth_repository signup paths that are still relevant.
const fs = require('fs');
...
// ── Register flow scope ─────────────────────────────────────────────────────
const registerPathSuffixes = [
  'lib/features/auth/presentation/pages/register_page.dart',
  'lib/features/auth/presentation/auth_validators.dart',
  'lib/features/auth/domain/models/signup_request.dart',
  // Shared with login but exercised by register too — included for visibility:
...
const EXCLUSIONS = [
  // Register page currently has no provably-uncoverable lines after the
  // rewrite, so the list is empty. Add entries here only with a written
  // reason if a line is unreachable from unit tests (e.g., decorator
  // metadata, debug-only closures, framework wiring).
];
...
// ── Collect register-flow files ─────────────────────────────────────────────
const parsed = records.map(parseRecord).filter(Boolean);
const registerFiles = parsed.filter((p) => {
  if (isGenerated(p.sf)) return false;
  return registerPathSuffixes.some((sfx) => p.sf.endsWith(sfx));
});
registerFiles.sort((a, b) => a.sf.localeCompare(b.sf));
...
console.log('');
console.log('Register Flow — Mobile Coverage Report');
...
for (const record of registerFiles) {
...
console.log(
  'TOTAL (register flow, excl. documented skips)'.padEnd(60) +
