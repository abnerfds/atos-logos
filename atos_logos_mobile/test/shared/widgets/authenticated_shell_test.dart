...
void main() {
  ...
  group('AuthenticatedShell - chrome', () {
    ...
    testWidgets(
      ...
      when(() => mockAuthCubit.state).thenReturn(
        const AuthState.authenticated(profile: emptyNameProfile),
      );
  ...
  group('AuthenticatedShell - bottom nav', () {
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (context, state) {
              ...
              return Scaffold(body: Text('HOME_ROUTE tab=${state.uri.queryParameters['tab']}'));
            },
          ),
          GoRoute(
            path: '/login',
    ...
    testWidgets(
      ...
      expect(
        semantics.flagsCollection.isSelected.name,
        'isTrue',
      );
    });
  });
  ...
  group('AuthenticatedShell - profile sheet', () {
    testWidgets('should open the profile bottom sheet when the avatar is tapped',
        (tester) async {
      await tester.pumpWidget(buildSubject());
